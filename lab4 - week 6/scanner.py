...
class Scanner:
    ...
    def __init__(self, programPath, tokensPath):
        ...
        self.operatorBuffer = ""
        self.int_const_fa = FiniteAutomaton("int_const.in")
        self.identifier_fa = FiniteAutomaton("identifier.in")
    ...
    def is_identifier_fa(self, token):
        return self.identifier_fa.check_sequence(token)

    def is_int_constant(self, token):
        return re.match(r"^(0|[-]?[1-9][0-9]*)$", token)
    
    def is_int_constant_fa(self, token):
        return self.int_const_fa.check_sequence(token)
